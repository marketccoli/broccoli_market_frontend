  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center justify-center w-full relative">
        ...
        <span></span>
        {data?.data.sellProduct.length ? "" : "판매중 아이템이 없습니다."}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 p-2 max-h-full  w-full max-w-[1000px]">
          {data?.data.sellProduct.length ? data.data.sellProduct.map((product) => <ProductCard key={product.product_id} product={product} />) : ""}
        </div>
      </div>
    </>
  );
